use fiber_dashbord_backend::{
    ...
    pg_write_types::{ChannelInfoDBSchema, from_rpc_to_db_schema, init_global_cache, insert_batch},
    ...
    rt.block_on(async move {
        ...
        tokio::spawn(async move {
            loop {
                ...
                if count == 0 {
                    sqlx::query(
                        "CALL refresh_continuous_aggregate('online_nodes_hourly', NULL, NULL)",
                    )
                    .execute(pool)
                    .await
                    .expect("Failed to refresh continuous aggregate");
                    sqlx::query(
                        "CALL refresh_continuous_aggregate('online_channels_hourly', NULL, NULL)",
                    )
                    .execute(pool)
                    .await
                    .expect("Failed to refresh continuous aggregate");
